...
- **`Parse`** turns CommonMark (with the GFM table and strikethrough
  extensions) into a `*richdoc.Document`. Parsing delegates to the reference Go
  Markdown library, [goldmark](https://github.com/yuin/goldmark); this package
  never hand-rolls a CommonMark parser. It walks goldmark's AST and builds the
  richdoc tree.
...
| ------------------------------------- | --------------------------------------------- |
| `Heading` (ATX and setext)            | `Heading{Level, Inlines}`                     |
...
| ------------------- | ---------------------------------------------------------- |
| `Heading`           | `#`…`######` ATX heading (level clamped to 1–6)            |
